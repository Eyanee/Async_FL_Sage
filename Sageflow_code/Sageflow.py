...
from otherGroupingMethod import get_key_list, preGrouping, restoreWeight, AFA
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i in range(args.staleness + 1):
            if i != 0:
                if args.update_rule == 'Sageflow':
                    print("i is ",i)
                    print("len(local_weights_delay[i] is ",len(local_weights_delay[i]))
        ...
        if args.update_rule == 'Sageflow':
            ...
            global_update= AFA(param_updates, device)
